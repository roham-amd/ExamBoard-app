...
import { createTimetableQuery, parseTimetableFilters } from '@/src/lib/public-timetable-filters'
...
export default async function PublicTimetablePage({ params, searchParams }: PageProps) {
  const filters = parseTimetableFilters(searchParams)
  const queryString = createTimetableQuery(filters).toString()
  const data = await fetchPublicTimetable(params.termId, queryString)
