...
export default function RoomsPage() {
  ...
  const onSubmit = form.handleSubmit(async values => {
    ...
    try {
      if (dialogMode === 'edit' && selectedRoom) {
        await updateMutation.mutateAsync({ id: selectedRoom.id, data: payload })
  ...
  const columns: DataTableColumn<Room>[] = [
    ...
    { header: t('columns.campus'), accessor: item => item.campus ?? t('unknown') },
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('form.capacity')}</label>
                <input
                  type="number"
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('form.code')}</label>
                <input
                  type="text"
                  {...form.register('code')}
                  disabled={isSubmitting}
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('form.campus')}</label>
                <input
                  type="text"
                ...
                {form.formState.errors.campus ? (
                  <p className="text-xs text-destructive">{form.formState.errors.campus.message}</p>
                ) : null}
              </div>
              <div className="space-y-1 sm:col-span-2">
