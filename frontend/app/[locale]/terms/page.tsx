...
export default function TermsPage() {
  ...
  const onSubmit = form.handleSubmit(async values => {
    ...
    try {
      if (dialogMode === 'edit' && selectedTerm) {
        await updateMutation.mutateAsync({ id: selectedTerm.id, data: payload })
  ...
  const columns: DataTableColumn<Term>[] = [
    ...
    },
    {
      ...
      accessor: item => (item.published_at ? t('status.published') : t('status.draft'))
    },
    {
        ...
        <DialogContent className="sm:max-w-xl">
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('form.slug')}</label>
                <input
                  type="text"
                  {...form.register('slug')}
                  disabled={isSubmitting || isPublished}
              ...
              <div className="flex items-center gap-2 sm:col-span-2">
                ...
                />
                <label className="text-sm text-foreground" htmlFor="term-is-active">
                  {t('form.isActive')}
                </label>
              </div>
            </div>
