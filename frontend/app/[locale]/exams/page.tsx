...
import { useExamOwnersQuery, useExamsQuery, useTermsQuery } from '@/src/lib/queries'
import {
...
export default function ExamsPage() {
  ...
  const onSubmit = form.handleSubmit(async values => {
    ...
    try {
      if (dialogMode === 'edit' && selectedExam) {
        await updateMutation.mutateAsync({ id: selectedExam.id, data: payload })
  ...
  const columns: DataTableColumn<Exam>[] = [
    ...
    { header: t('columns.expected'), accessor: item => item.expected_students },
    { header: t('columns.term'), accessor: item => item.term_name },
    {
      header: t('columns.duration'),
      accessor: item => t('durationMinutes', { value: item.duration_minutes })
    },
    {
        ...
        <DialogContent className="sm:max-w-xl">
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('form.courseCode')}</label>
                <input
                  type="text"
                ...
                {form.formState.errors.course_code ? (
                  <p className="text-xs text-destructive">{form.formState.errors.course_code.message}</p>
                ) : null}
              </div>
              <Controller
                name="owner"
                ...
                render={({ field }) => (
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-foreground">{t('form.owner')}</label>
                    <select
                    ...
                    {form.formState.errors.owner ? (
                      <p className="text-xs text-destructive">{form.formState.errors.owner.message}</p>
                    ) : null}
                  </div>
                )}
              />
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('form.expectedStudents')}</label>
                <input
                  type="number"
                  min={1}
                  {...form.register('expected_students', { valueAsNumber: true })}
                  disabled={isSubmitting}
                  className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                />
                {form.formState.errors.expected_students ? (
                  <p className="text-xs text-destructive">{form.formState.errors.expected_students.message}</p>
                ) : null}
              </div>
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('form.durationMinutes')}</label>
                <input
                  type="number"
                  min={10}
                  step={5}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-foreground">{t('form.term')}</label>
                    <select
                      value={field.value ? String(field.value) : ''}
                      onChange={event => field.onChange(Number(event.target.value))}
