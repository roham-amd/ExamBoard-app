...
import { useAllocationsQuery, useExamsQuery, useRoomsQuery } from '@/src/lib/queries'
import {
...
const mapAllocationToValues = (allocation: Allocation): AllocationFormValues => ({
...
export default function AllocationsPage() {
  ...
  const onSubmit = form.handleSubmit(async values => {
    ...
    try {
      if (dialogMode === 'edit' && selectedAllocation) {
        await updateMutation.mutateAsync({ id: selectedAllocation.id, data: payload })
  ...
  const columns: DataTableColumn<Allocation>[] = [
    { header: t('columns.exam'), accessor: item => item.exam_title },
    {
      ...
      accessor: item =>
        item.rooms.length > 0 ? item.rooms.map(room => room.name).join('، ') : t('unassigned')
    },
    {
        ...
        <DialogContent className="sm:max-w-xl">
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            <div className="grid gap-4 sm:grid-cols-2">
              <Controller
                ...
                render={({ field }) => (
                  <div className="space-y-1 sm:col-span-2">
                    <label className="text-sm font-medium text-foreground">{t('form.exam')}</label>
                    <select
                    ...
                    {form.formState.errors.exam ? (
                      <p className="text-xs text-destructive">{form.formState.errors.exam.message}</p>
                    ) : null}
                  </div>
                )}
              />
              ...
              />
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('form.allocatedSeats')}</label>
                <input
                  type="number"
                  min={1}
                ...
                {form.formState.errors.allocated_seats ? (
                  <p className="text-xs text-destructive">{form.formState.errors.allocated_seats.message}</p>
                ) : null}
              </div>
              <div className="space-y-1 sm:col-span-2">
