...
const prependItem = <T>(
  ...
  const queries = queryClient.getQueriesData<PaginatedResponse<T>>({ queryKey: resourceKey })
...
const replaceItem = <T extends Identifiable>(
  ...
  for (const [key, data] of queries) {
    ...
    queryClient.setQueryData(key, {
      ...data,
      results: data.results.map(entry => (entry.id === item.id ? item : entry))
    })
  }
}
...
export const useCreateAllocationMutation = () => {
  ...
  return useMutation({
    ...
    onSettled: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.allocations.all })
    }
  })
}
...
export const useUpdateAllocationMutation = () => {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, data }: { id: number; data: AllocationFormValues }) => {
...
export const useUpdateBlackoutMutation = () => {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, data }: { id: number; data: BlackoutFormValues }) => {
...
export const useUpdateHolidayMutation = () => {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, data }: { id: number; data: HolidayFormValues }) => {
