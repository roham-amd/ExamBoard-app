...
export function parseTokenPayload(data: unknown): TokenPayload {
  ...
  return {
    access: typeof record.access === 'string' ? (record.access as string) : undefined,
    refresh: typeof record.refresh === 'string' ? (record.refresh as string) : undefined,
    access_expires_in:
      typeof record.access_expires_in === 'number' ? (record.access_expires_in as number) : undefined,
    refresh_expires_in:
      typeof record.refresh_expires_in === 'number' ? (record.refresh_expires_in as number) : undefined
  }
}

export function setAuthCookies(payload: TokenPayload) {
  const store = cookies()
...
export function clearAuthCookies() {
  const store = cookies()
  const base = { httpOnly: true, sameSite: 'lax' as const, secure: isProd, path: '/', maxAge: 0 }
