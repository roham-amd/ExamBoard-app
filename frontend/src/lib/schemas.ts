...
const isoDate = z
  ...
  .refine(value => !Number.isNaN(Date.parse(value)), { message: 'فرمت تاریخ معتبر نیست' })
...
export const termFormSchema = z
  .object({
    ...
    slug: z
      ...
      .regex(/^[a-z0-9-]+$/i, 'شناسه کوتاه فقط شامل حروف، اعداد و خط تیره باشد'),
    starts_at: isoDate,
    ends_at: isoDate,
  ...
  .superRefine((values, ctx) => {
    if (new Date(values.ends_at) < new Date(values.starts_at)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['ends_at'], message: 'تاریخ پایان باید بعد از تاریخ شروع باشد' })
    }
  })

export const roomFormSchema = z.object({
  code: z.string().min(1, 'کد سالن الزامی است'),
  name: z.string().min(2, 'نام سالن حداقل دو نویسه باشد'),
  capacity: z.number({ invalid_type_error: 'ظرفیت باید عددی باشد' }).int().positive('ظرفیت باید مثبت باشد'),
...
export const examFormSchema = z.object({
  ...
  course_title: z.string().min(3, 'عنوان درس حداقل سه نویسه باشد'),
  term: z.number({ invalid_type_error: 'شناسه نیمسال نامعتبر است' }).int().positive(),
  starts_at: isoDate,
  duration_minutes: z.number({ invalid_type_error: 'مدت زمان باید عددی باشد' }).int().positive(),
  status: z.enum(['draft', 'confirmed', 'published']).default('draft')
})

export const allocationFormSchema = z.object({
  exam: z.number({ invalid_type_error: 'شناسه امتحان نامعتبر است' }).int().positive(),
  room: z.number({ invalid_type_error: 'شناسه سالن نامعتبر است' }).int().positive(),
  seats_reserved: z
