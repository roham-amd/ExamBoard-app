...
const mapAllocationsToSegments = (
  ...
  return allocations.map((allocation) => {
    ...
    const durationMinutes = ensureMinDuration(
...
export function PublicTimetable(props: PublicTimetableProps) {
  ...
  const filteredAllocations = useMemo(() => {
    ...
    const roomSet = activeFilters.rooms.length
      ? new Set(activeFilters.rooms)
      : null;
  ...
  const allocationByRoom = useMemo(() => {
    ...
    const segments = mapAllocationsToSegments(
      filteredAllocations,
      start.valueOf(),
      end.valueOf(),
    );
  ...
  const updateFilter = <Key extends keyof TimetableFilters>(
    key: Key,
    value: TimetableFilters[Key],
  ) => {
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            بازهٔ ترم: {jalaliFormat(term.starts_at, "D MMMM YYYY")} تا{" "}
            {jalaliFormat(term.ends_at, "D MMMM YYYY")}
          </p>
          {generatedAt ? (
