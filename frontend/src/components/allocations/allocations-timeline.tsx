...
const TimelineItem = memo(
  ({
    ...
    const widthPercent =
      totalMs > 0
        ? ((allocationEnd.getTime() - allocationStart.getTime()) / totalMs) *
          100
        : 0;
    const leftPercent =
      totalMs > 0
        ? ((allocationStart.getTime() - rangeStart.getTime()) / totalMs) * 100
        : 0;
      ...
      >
        ...
        {isSelected ? (
          <div
            ...
            aria-label={nudgeLabels.keyboardHint}
          >
  ...
  ({
    ...
    return (
      <div className="contents">
        ...
        <div
          ref={setNodeRef}
          className="relative min-h-[120px] rounded-md border border-dashed border-border bg-background"
        >
          {segments.length === 0 ? (
          ...
          {segments.map((segment) => (
            <TimelineItem
              ...
              isSelected={
                selected?.allocationId === segment.allocation.id &&
                selected?.roomId === segment.room.id
              }
              onSelect={() =>
                onSelect({
                  allocationId: segment.allocation.id,
                  roomId: segment.room.id,
                })
              }
              onNudge={(kind, delta) =>
                void onNudge(segment.allocation, segment.room.id, kind, delta)
              }
...
export function AllocationsTimeline() {
  ...
  const [rangeDraft, setRangeDraft] = useState<TimelineRange>(() =>
    getDefaultRange(),
  );
  ...
  const segmentsByRoom = useMemo(() => {
    const map = new Map<
      number,
      Array<{ allocation: Allocation; room: AllocationRoomSummary }>
    >();
  ...
  const activeSelection = useMemo(() => {
    ...
    return segments.find(
      (segment) =>
        segment.allocation.id === selected.allocationId &&
        segment.room.id === selected.roomId,
    );
  }, [segments, selected]);
  ...
  const calculateCapacityEstimate = useCallback(
    (allocation: Allocation, roomId: number, startsAt: Date, endsAt: Date) => {
      ...
      const existingSeats = overlapping.reduce(
        (total, segment) => total + segment.allocation.allocated_seats,
        0,
      );
  ...
  const onDragEnd = useCallback(
    async ({ active, delta, over }: DragEndEvent) => {
      ...
      if (!metadata) return;
      const allocation = allocationsQuery.data?.results.find(
        (item) => item.id === metadata.allocationId,
      );
      if (!allocation) return;
      const originalStart = new Date(metadata.originalStartsAt);
      const originalEnd = new Date(metadata.originalEndsAt);
      const targetRoomId =
        over && typeof over.id === "string" && over.id.startsWith("room-")
  ...
  const handleRangeSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (
      !Number.isFinite(draftStart.getTime()) ||
      !Number.isFinite(draftEnd.getTime()) ||
      draftStart >= draftEnd
    ) {
      toast({
        title: t("rangeInvalid.title"),
  ...
  const selectedRoomCapacity = activeSelection
    ? (roomMap.get(activeSelection.room.id)?.capacity ?? 0)
    : 0;
  const meterPercent =
    selectedRoomCapacity > 0
      ? Math.min(100, (maxCapacityValue / selectedRoomCapacity) * 100)
      : 0;
  ...
  const handleSelect = useCallback(
    (selection: { allocationId: number; roomId: number }) => {
    ...
    <section className="space-y-4 rounded-lg border border-border bg-card p-4">
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-[120px_1fr] gap-2 text-xs font-medium text-muted-foreground">
          ...
          <div
            ref={timelineRef}
            className="relative overflow-hidden rounded-md border border-dashed border-border bg-muted/40"
          >
