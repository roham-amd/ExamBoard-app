...
export function DataTable<T>({ data, columns, isLoading, error, onRetry, total }: DataTableProps<T>) {
  const t = useTranslations('dataTable')
  const errorContent = useMemo(() => (error ? resolveErrorContent(error) : null), [error])
  ...
  if (errorContent) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-destructive/40 bg-destructive/10 p-6 text-center">
        <div>
          <p className="text-lg font-semibold text-destructive">{errorContent.title}</p>
          <p className="text-sm text-destructive/80">{errorContent.description}</p>
        </div>
        {onRetry ? (
            ...
            <tr>
              {columns.map(column => (
                <th
                  key={column.header}
                  scope="col"
                  className={cn('px-4 py-3 text-start text-xs font-medium uppercase tracking-wide', column.className)}
                >
                  {column.header}
                </th>
              ))}
